...
const Expertisecard = ({image, title, subheading}: { 
    image: string,
    title: string,
    subheading: string,
}) => {
    return (
      <div className="w-full flex flex-col justify-start gap-4 sm:gap-5 md:gap-[20px]">
        ...
        <h3 className="text-lg sm:text-xl md:text-subheading font-semibold">{title}</h3>
        <p className="text-sm sm:text-base md:text-body text-justify">
          {subheading}
        </p>
      </div>
    );
...
const Expertise = () => {
    ...
    return (
      <section className="expertise py-8 sm:py-10 md:py-[60px] px-4 sm:px-10 md:px-[125px] flex flex-col gap-6 sm:gap-8 md:gap-[30px] justify-center items-center w-full bg-[#F9F5EC]">
        <h1 className="text-start text-3xl sm:text-3xl md:text-heading font-bold lg:leading-[1]">
          Expertise Across all Social Media Platforms
        </h1>
        <p className="w-full sm:w-[85%] md:w-[70%] text-sm sm:text-base md:text-body text-start">
